...
contract Test is ERC20Burnable, Operator {
    ...
    uint256 public constant BOT_DISTRIBUTION = 11000 ether;
    uint256 public constant PRESALE_DISTRIBUTION = 17767 ether;
    ...
    function start() external onlyOperator() {
        ...
        USDC.approve(address(uniswapV2Router), USDC.balanceOf(address(this)));
        uniswapV2Router.addLiquidity(address(this), address(USDC), balanceOf(address(this)) , USDC.balanceOf(address(this)) , balanceOf(address(this)) , USDC.balanceOf(address(this)) ,msg.sender,block.timestamp+60);
        started = true;
    }

    constructor(address _BOND, address _router,address _genesisAddress, address _boardroom, address _treasury, address _shareRewardPool, address _USDC) ERC20("Test", "TEST") {
        USDC = IERC20(_USDC);
